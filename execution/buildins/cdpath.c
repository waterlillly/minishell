...
/*   Updated: 2024/07/28 18:16:22 by lbaumeis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cd_home(t_buildins *vars)
{
	...
	if (ft_strcmp(vars->home, vars->pwd))
		return (0);
	vars->oldpwd = vars->pwd;
	vars->pwd = ft_strdup(vars->home);
	return (chdir("~"));
}
...
char	*go_full_path(t_buildins *vars, char **token, int x, char *temp)
{
	...
	{
		vars->oldpwd = vars->pwd;
		vars->pwd = ft_strdup(token[x + 1]);
		temp = ft_strdup(token[x + 1]);
	}
	else
...
char	*fill_path(t_buildins *vars, char **token, int x)
{
	...
	temp = NULL;
	if (token[x + 1][0] == '.' && token[x + 1][1] == '.' && token[x + 1][2] == '\0')
		temp = go_back(vars, temp);
	else if (token[x + 1][0] == '.' && token[x + 1][1] == '\0')
		return (NULL);
	else if (token[x + 1][0] == '/' && token[x + 1][1] != '\0'
		&& token[x + 1][1] != '/')
		temp = go_full_path(vars, token, x, temp);
	else if (token[x + 1][0] == '/')
		temp = go_slash(vars, token, x, temp);
	else
